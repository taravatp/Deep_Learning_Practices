...
def load_cifar10(transpose=False):
    ...
    path = get_file(dirname, origin=origin, untar=True)  # data/cifar-10-batches-py.tar.gz
    ...
    # 50000 training set
    for i in range(1, 6):  # we are going to have 6 data batches
...
def get_file(fname, origin, untar=False, cache_dir='data'):
    datadir = os.path.join(cache_dir)  # path = /data
    if not os.path.exists(datadir):
        os.makedirs(datadir)

    if untar:
        untar_fpath = os.path.join(datadir, fname)  # path = data/cifar-10-batches-py
        fpath = untar_fpath + '.tar.gz'  # path = data/cifar-10-batches-py.tar.gz
    else:
        fpath = os.path.join(datadir, fname)

    print('File path: %s' % fpath)  # data/cifar-10-batches-py.tar.gz
    if not os.path.exists(fpath):
        ...
        try:
            try:
                urlretrieve(origin, fpath)  # downloads the remote data directly to the local
...
def _rgb2label(rgb_images, colours):
    ...
    Returns:
      result: int numpy array of shape [B, 1, H, W]
    """
    num_colours = np.shape(colours)[0]  # 24 ta color darim
    rgb_images = np.expand_dims(rgb_images, 0)  # darim ye bod be tahe RGB image ha ezafe mikonim
...
def custom_plot(input, gtlabel, output, colours, path, visualize, compare_bilinear=False):
    """
    ...
    gtcolor = cat2rgb(gtlabel[:10, :, :], colours)
    predcolor = cat2rgb(output[:10, :, :], colours)
    ...
    if compare_bilinear:
        downsize_module = nn.Sequential(nn.AvgPool2d(2),
                                        ...
                                        nn.Upsample(scale_factor=2, mode='bilinear'))
        gt_input = np.transpose(gtcolor, [0, 2, 1])
